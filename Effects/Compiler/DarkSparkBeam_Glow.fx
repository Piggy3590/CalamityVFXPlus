...
float4 DarkSparkGlow(VertexShaderOutput input) : COLOR0
{
    ...
    float alphaMask = tex2D(samplerOnTex, float2(UV.x + (1.0f * uTime), UV.y)).a;
    float4 input_color = float4(baseColor, alphaMask);
    ...
    return combined2 * input.Color.a * glowFade;
}
